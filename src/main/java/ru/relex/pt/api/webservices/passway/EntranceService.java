...
import ru.relex.pt.database.NotAuthrizedException;
...
public class EntranceService {
	...
	public PassesResponse getPasses(@QueryParam("since") @DefaultValue("0") long since) {
		...
		for(PwEntrance entrance:service.getEntrances(since))
		{
			...
			newPass.setStatus(entrance.getStatus());
			passes.add(newPass);
		}
	...
	@Produces("application/json")
	public Pass registerPass(Pass pass) throws NotAuthrizedException {
	...
	@PUT
	@Produces("application/json")
	public void processPass(PassProcessInfo passInfo) throws NotFoundEntranceException, UnknownStatusException, IllegalOperationSequenceException, InvalidAbsentTypeException {
	    service.updateEntrance(passInfo.getId(), passInfo.getStatus(), passInfo.getAbsentTimeMin(), passInfo.getAbsentType());
	}
}
